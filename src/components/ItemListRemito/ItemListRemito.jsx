                    ...
                    {filterState === ''
                        ...
                        ? remitos.map((remito) => {
                            ...
                            return (
                                ...
                                <tbody>
                                    <th className="border-bottom my-2" key={remito.id} scope="row">{remito.id}</th>
                                    
                                    <td className="border-bottom" key={remito.id}>{remito.departamento_id}</td>
                                    <td className="border-bottom" key={remito.id}>{remito.fecha_recepcion}</td>
                                    <td className="border-bottom" key={remito.id}>{remito.remito}</td>
                                    <td className="border-bottom" key={remito.id}>{remito.fecha_recepcion_dti}</td>
                                    <td className="border-bottom" key={remito.id}><i className="bi bi-file-pdf iconRemito"> <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" className="bi bi-file-pdf iconRemito" viewBox="0 0 16 16">
                        ...
                        : remitos.filter(remito => remito.departamento_id.toString().toLowerCase().includes(filterState.toLowerCase()) ||
                            ...
                            return (
                                ...
                                <tbody>
                                    <th key={remito.id} scope="row">{remito.id}</th>
                                    <td className="border-bottom" key={remito.id}>{remito.departamento_id}</td>
                                    <td className="border-bottom" key={remito.id}>{remito.fecha_recepcion}</td>
                                    <td className="border-bottom" key={remito.id}>{remito.remito}</td>
                                    <td className="border-bottom" key={remito.id}>{remito.fecha_recepcion_dti}</td>
